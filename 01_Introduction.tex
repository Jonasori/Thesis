...
% REWORK: "How about a reference, or even just a footnote to the NASA exoplanet database? Could also include a reference to the statistic that there is at least one planet per star according to Kepler… I think the first author was Erik Petigura? "
...
\begin{figure}[htp]
  \hspace*{\fill}%
  \subcaptionbox{\label{fig:m1map_hco}}{\includegraphics[width=0.5\linewidth]{moment1_hco-data.pdf}}\hfill%
  \subcaptionbox{\label{fig:m1map_hcn}}{\includegraphics[width=0.5\linewidth]{moment1_hcn-data.pdf}}\vfill%
  \subcaptionbox{\label{fig:m1map_co}}{\includegraphics[width=0.5\linewidth]{moment1_co-data.pdf}}\hfill%
  \subcaptionbox{\label{fig:m1map_cs}}{\includegraphics[width=0.5\linewidth]{moment1_cs-data.pdf}}\hfill%
  \hspace*{\fill}%
  \label{fig:m1map}
  \captionof{figure}{Moment-1 maps of HCO$^+$(4-3), HCN(4-3), CO(3-2), and CS(7-6) emission (left to right) in the d253-1536 system, observed with ALMA's Band 7. Each map shows intensity-weighted velocity, which allows us to trace the disks' kinematics. REWORK: considering making this a 2x2 grid, instead of 4 across.}
\end{figure}
% REWORK: "Again, I see both moment 1 *and* moment 0 here (presumably that’s what the contours are for).  Need to include more description in the caption, including the contour levels.  Should include more contour levels in the HCO+ panel — right now it looks like they stop halfway up the disk.  I think changing to a 2x2 grid is a fine idea."
